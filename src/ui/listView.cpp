...
ListView::ListView(const irect &contentRect, int page)
    : _contentRect(contentRect), _shownPage(page) {
  _entries.clear();
  ...
  _pageIcon = iRect(_contentRect.w - footerWidth * 2,
                    _contentRect.h + _contentRect.y - _footerHeight,
                    contentRect.w / 10, _footerHeight, ALIGN_CENTER);

  _firstPageButton =
      iRect(_contentRect.x, _contentRect.h + _contentRect.y - _footerHeight,
            contentRect.w / 8, _footerHeight, ALIGN_CENTER);

  _prevPageButton = iRect(_contentRect.x + footerWidth * 3,
                          _contentRect.h + _contentRect.y - _footerHeight,
                          contentRect.w / 8, _footerHeight, ALIGN_CENTER);

  _nextPageButton = iRect(_contentRect.x + footerWidth * 6,
                          _contentRect.h + _contentRect.y - _footerHeight,
                          contentRect.w / 8, _footerHeight, ALIGN_CENTER);

  _lastPageButton = iRect(_contentRect.x + footerWidth * 9,
                          _contentRect.h + _contentRect.y - _footerHeight,
                          contentRect.w / 8, _footerHeight, ALIGN_CENTER);
}
...
void ListView::reDrawCurrentEntry() {
  ...
  _entries.at(_selectedEntry)
      ->draw(_entryFont, _entryFontBold, _entryFontHeight);
  updateEntry(_selectedEntry);
}

void ListView::invertCurrentEntryColor() {
  InvertAreaBW(_entries.at(_selectedEntry)->getPosition().x,
               _entries.at(_selectedEntry)->getPosition().y,
               _entries.at(_selectedEntry)->getPosition().w,
               _entries.at(_selectedEntry)->getPosition().h);
  updateEntry(_selectedEntry);
}
...
bool ListView::checkIfEntryClicked(int x, int y) {
  if (IsInRect(x, y, &_firstPageButton)) {
    ...
    for (unsigned int i = 0; i < _entries.size(); i++) {
      if (_entries.at(i)->getPage() == _shownPage &&
          IsInRect(x, y, &_entries.at(i)->getPosition()) == 1) {
        _selectedEntry = i;
        return true;
      }
    }
...
void ListView::updateEntry(int entryID) {
  PartialUpdate(_entries.at(entryID)->getPosition().x,
                _entries.at(entryID)->getPosition().y,
                _entries.at(entryID)->getPosition().w,
                _entries.at(entryID)->getPosition().h);
}
...
void ListView::actualizePage(int pageToShow) {
  if (pageToShow > _page) {
    ...
    Message(ICON_INFORMATION, "Info", "You are already on the first page.",
            1200);
  } else {
    _shownPage = pageToShow;
    FillArea(_contentRect.x, _contentRect.y, _contentRect.w, _contentRect.h,
             WHITE);
    drawEntries();
    drawFooter();
    PartialUpdate(_contentRect.x, _contentRect.y, _contentRect.w,
                  _contentRect.h);
  }
}
