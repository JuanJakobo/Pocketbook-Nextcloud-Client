...
LoginView::LoginView(const irect &contentRect) : _contentRect(contentRect)
{
    ...
    FontDeleter fontDeleter;
    _loginFont =
        std::unique_ptr<ifont, FontDeleter>(OpenFont("LiberationMono", _loginFontHeight, FONT_STD), fontDeleter);
    SetFont(_loginFont.get(), BLACK);
    FillAreaRect(&_contentRect, WHITE);

    _urlButton = iRect(beginX, beginY, contentWidth, contents, ALIGN_CENTER);
    DrawTextRect(_urlButton.x, _urlButton.y - _loginFontHeight - _loginFontHeight / 2, _urlButton.w, _urlButton.h,
                 "Server address:", ALIGN_LEFT);
    ...
    DrawTextRect(_usernameButton.x, _usernameButton.y - _loginFontHeight - _loginFontHeight / 3, _usernameButton.w,
                 _usernameButton.h, "Username:", ALIGN_LEFT);
    ...
    DrawTextRect(_passwordButton.x, _passwordButton.y - _loginFontHeight - _loginFontHeight / 4, _passwordButton.w,
                 _passwordButton.h, "Password:", ALIGN_LEFT);
    DrawRect(_passwordButton.x - 1, _passwordButton.y - 1, _passwordButton.w + 2, _passwordButton.h + 2, BLACK);

    _ignoreCertButton =
        iRect(_contentRect.w - 2 * checkBoxWidth, beginY + 6 * contents, checkBoxWidth, contents, ALIGN_CENTER);
    DrawTextRect(beginX, _ignoreCertButton.y, contentWidth, _ignoreCertButton.h, "Ignore Cert (insecure):", ALIGN_LEFT);
...
int LoginView::logginClicked(int x, int y)
{
    ...
    if (IsInRect(x, y, &_urlButton))
    {
        ...
        OpenKeyboard("https://domainname or WebDAV Url", &_temp[0], KEYBOARD_STRING_LENGHT - 1, KBD_NORMAL,
    ...
    else if (IsInRect(x, y, &_ignoreCertButton))
    {
        ...
        if (_ignoreCert)
            FillArea(_ignoreCertButton.x - 1, _ignoreCertButton.y - 1, _ignoreCertButton.w + 2, _ignoreCertButton.h + 2,
                     BLACK);
        else
            DrawRect(_ignoreCertButton.x - 1, _ignoreCertButton.y - 1, _ignoreCertButton.w + 2, _ignoreCertButton.h + 2,
                     BLACK);
