...
LoginView::LoginView(const irect &contentRect) : _contentRect(contentRect) {
  ...
  DrawTextRect(_urlButton.x,
               _urlButton.y - _loginFontHeight - _loginFontHeight / 2,
               _urlButton.w, _urlButton.h, "Server address:", ALIGN_LEFT);
  DrawRect(_urlButton.x - 1, _urlButton.y - 1, _urlButton.w + 2,
           _urlButton.h + 2, BLACK);

  _usernameButton = iRect(beginX, beginY + 2 * contents, contentWidth, contents,
                          ALIGN_CENTER);
  DrawTextRect(_usernameButton.x,
               _usernameButton.y - _loginFontHeight - _loginFontHeight / 3,
               _usernameButton.w, _usernameButton.h, "Username:", ALIGN_LEFT);
  DrawRect(_usernameButton.x - 1, _usernameButton.y - 1, _usernameButton.w + 2,
           _usernameButton.h + 2, BLACK);

  _passwordButton = iRect(beginX, beginY + 4 * contents, contentWidth, contents,
                          ALIGN_CENTER);
  DrawTextRect(_passwordButton.x,
               _passwordButton.y - _loginFontHeight - _loginFontHeight / 4,
               _passwordButton.w, _passwordButton.h, "Password:", ALIGN_LEFT);
  DrawRect(_passwordButton.x - 1, _passwordButton.y - 1, _passwordButton.w + 2,
           _passwordButton.h + 2, BLACK);
  ...
  _loginButton = iRect(beginX, beginY + 8 * contents, contentWidth, contents,
                       ALIGN_CENTER);
...
int LoginView::logginClicked(int x, int y) {
  ...
  if (IsInRect(x, y, &_urlButton)) {
    ...
    OpenKeyboard("https://domainname or WebDAV Url", &_temp[0],
                 KEYBOARD_STRING_LENGHT - 1, KBD_NORMAL,
    ...
    if (!_username.empty())
      _temp = _username;
    _temp.resize(KEYBOARD_STRING_LENGHT);
    OpenKeyboard("Username", &_temp[0], KEYBOARD_STRING_LENGHT, KBD_NORMAL,
                 &keyboardHandlerStatic);
    return 1;
  } else if (IsInRect(x, y, &_passwordButton)) {
    _target = KeyboardTarget::IPASSWORD;
    _temp.resize(KEYBOARD_STRING_LENGHT);
    OpenKeyboard("Password", &_temp[0], KEYBOARD_STRING_LENGHT, KBD_PASSWORD,
                 &keyboardHandlerStatic);
    ...
    if (_ignoreCert)
      FillArea(_ignoreCertButton.x - 1, _ignoreCertButton.y - 1,
               _ignoreCertButton.w + 2, _ignoreCertButton.h + 2, BLACK);
    else
      DrawRect(_ignoreCertButton.x - 1, _ignoreCertButton.y - 1,
               _ignoreCertButton.w + 2, _ignoreCertButton.h + 2, BLACK);

    PartialUpdate(_ignoreCertButton.x, _ignoreCertButton.y, _ignoreCertButton.w,
                  _ignoreCertButton.h);

    return 1;
  } else if (IsInRect(x, y, &_loginButton)) {
    if (_username.empty() || _password.empty() || _url.empty()) {
      Message(ICON_ERROR, "Error", "Please set url, username and password.",
              1200);
      return 1;
    }
