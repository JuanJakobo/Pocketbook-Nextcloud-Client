...
FileView::FileView(const irect &contentRect, const vector<FileItem> &files,
                   int page)
  ...
  while (i < entrycount) {
    auto entrySize =
        TextRectHeight(contentRect.w, files.at(i).name.c_str(), 0) +
    ...
    _entries.emplace_back(std::unique_ptr<FileViewEntry>(
        new FileViewEntry(_page, rect, files.at(i))));
