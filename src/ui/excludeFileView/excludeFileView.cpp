...
ExcludeFileView::ExcludeFileView(const irect &contentRect) : _contentRect(contentRect)
{
    ...
    DrawTextRect(_extensionListButton.x, _extensionListButton.y - _fontHeight - _fontHeight / 2, _extensionListButton.w,
                 _extensionListButton.h, "Extensions:", ALIGN_LEFT);
    DrawString(_extensionListButton.x, _extensionListButton.y, _extensionList.c_str());
    DrawRect(_extensionListButton.x - 1, _extensionListButton.y - 1, _extensionListButton.w + 2,
             _extensionListButton.h + 2, BLACK);

    _regexButton = iRect(beginX, beginY + 2 * contents, contentWidth, contents, ALIGN_CENTER);
    DrawTextRect(_regexButton.x, _regexButton.y - _fontHeight - _fontHeight / 3, _regexButton.w, _regexButton.h,
                 "File-Regex:", ALIGN_LEFT);
    ...
    DrawTextRect(_folderRegexButton.x, _folderRegexButton.y - _fontHeight - _fontHeight / 3, _folderRegexButton.w,
                 _folderRegexButton.h, "Folder-Regex:", ALIGN_LEFT);
    DrawString(_folderRegexButton.x, _folderRegexButton.y, _folderRegex.c_str());
    DrawRect(_folderRegexButton.x - 1, _folderRegexButton.y - 1, _folderRegexButton.w + 2, _folderRegexButton.h + 2,
             BLACK);

    _startFolderButton = iRect(beginX, beginY + 6 * contents, contentWidth, contents, ALIGN_CENTER);
    DrawTextRect(_startFolderButton.x, _startFolderButton.y - _fontHeight - _fontHeight / 3, _startFolderButton.w,
                 _startFolderButton.h, "Start folder:", ALIGN_LEFT);
    DrawString(_startFolderButton.x, _startFolderButton.y, _startFolder.c_str());
    DrawRect(_startFolderButton.x - 1, _startFolderButton.y - 1, _startFolderButton.w + 2, _startFolderButton.h + 2,
             BLACK);

    _invertMatchButton =
        iRect(_contentRect.w - 2 * checkBoxWidth, beginY + 8 * contents, checkBoxWidth, contents, ALIGN_CENTER);
    DrawTextRect(beginX, _invertMatchButton.y, contentWidth, _invertMatchButton.h,
                 "Only include matching:", ALIGN_LEFT);
    DrawRect(_invertMatchButton.x - 1, _invertMatchButton.y - 1, _invertMatchButton.w + 2, _invertMatchButton.h + 2,
             BLACK);
    if (_invertMatch)
    {
        FillArea(_invertMatchButton.x - 1, _invertMatchButton.y - 1, _invertMatchButton.w + 2, _invertMatchButton.h + 2,
                 BLACK);
    }
    ...
    _cancelButton =
        iRect(beginX + contentWidth / 2, beginY + 10 * contents, contentWidth / 2 - 20, contents, ALIGN_CENTER);
...
int ExcludeFileView::excludeClicked(int x, int y)
{
    ...
    if (IsInRect(x, y, &_extensionListButton))
    {
        ...
        OpenKeyboard("docx,pdf,sdr", &_temp[0], EXCLUDE_FILE_KEYBOARD_STRING_LENGHT - 1, KBD_NORMAL,
    ...
    else if (IsInRect(x, y, &_regexButton))
    {
        ...
        if (!_regex.empty())
            _temp = _regex;
        _temp.resize(EXCLUDE_FILE_KEYBOARD_STRING_LENGHT);
        OpenKeyboard("Pre.*Post\\.pdf", &_temp[0], EXCLUDE_FILE_KEYBOARD_STRING_LENGHT, KBD_NORMAL,
                     &keyboardHandlerStatic);
        return 1;
    }
    else if (IsInRect(x, y, &_folderRegexButton))
    {
        ...
        if (!_folderRegex.empty())
            _temp = _folderRegex;
        _temp.resize(EXCLUDE_FILE_KEYBOARD_STRING_LENGHT);
        OpenKeyboard("/root/.*/test/", &_temp[0], EXCLUDE_FILE_KEYBOARD_STRING_LENGHT, KBD_NORMAL,
                     &keyboardHandlerStatic);
        return 1;
    }
    ...
    else if (IsInRect(x, y, &_invertMatchButton))
    {
        ...
        if (_invertMatch)
            FillArea(_invertMatchButton.x - 1, _invertMatchButton.y - 1, _invertMatchButton.w + 2,
                     _invertMatchButton.h + 2, BLACK);
        else
            DrawRect(_invertMatchButton.x - 1, _invertMatchButton.y - 1, _invertMatchButton.w + 2,
                     _invertMatchButton.h + 2, BLACK);
