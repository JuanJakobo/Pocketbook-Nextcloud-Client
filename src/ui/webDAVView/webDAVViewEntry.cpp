...
WebDAVViewEntry::WebDAVViewEntry(int page, const irect &position,
                                 const WebDAVItem &entry)
    : ListViewEntry(page, position), _entry(entry) {}

void WebDAVViewEntry::draw(const ifont *entryFont, const ifont *entryFontBold,
                           int fontHeight) {
  ...
  if (_entry.title.find("click to go back") != std::string::npos)
    DrawTextRect(_position.x, _position.y, _position.w, heightOfTitle,
                 _entry.title.c_str(), ALIGN_CENTER);
  else {
    DrawTextRect(_position.x, _position.y, _position.w, heightOfTitle,
                 _entry.title.c_str(), ALIGN_LEFT);
...

    {
      ...
      if (_entry.type == Itemtype::IFILE) {
        ...
        if (_entry.state != FileState::ILOCAL)
          DrawTextRect(_position.x, _position.y + heightOfTitle + height,
                       _position.w, fontHeight, _entry.fileType.c_str(),
                       ALIGN_LEFT);
      ...
      DrawTextRect(_position.x,
                   _position.y + heightOfTitle + fontHeight + height,
                   _position.w, fontHeight, text.c_str(), ALIGN_RIGHT);
      if (_entry.state != FileState::ILOCAL)
        DrawTextRect(_position.x,
                     _position.y + heightOfTitle + fontHeight + height,
      ...
      time = "Modified " + time;
      DrawTextRect(_position.x, _position.y + heightOfTitle, _position.w,
                   fontHeight, time.c_str(), ALIGN_LEFT);
    }
  }
