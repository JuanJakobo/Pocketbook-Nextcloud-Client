...
WebDAVView::WebDAVView(const irect &contentRect,
                       vector<WebDAVItem> &itemsUnfiltered, int page)
    : ListView(contentRect, page) {
  vector<WebDAVItem> items;
  std::copy_if(itemsUnfiltered.begin(), itemsUnfiltered.end(),
  ...
  string parentRootPath = rootPath.substr(
      0, rootPath.substr(0, rootPath.length() - 1).find_last_of("/") + 1);
  ...
  for (auto item : items) {
    ...
    entrySize += (item.title.find("click to go back") != std::string::npos)
                     ? 0.5 * _entryFontHeight
