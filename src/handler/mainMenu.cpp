...
MainMenu::MainMenu(const string &name) {
  ...
  _menuButtonRect = iRect(_mainMenuWidth * 2, _panelMenuBeginY, _mainMenuWidth,
                          _panelMenuHeight, ALIGN_RIGHT);

  _menuFont = OpenFont("LiberationMono-Bold", _panelMenuHeight / 2, FONT_STD);

  SetFont(_menuFont, BLACK);
  DrawTextRect(0, _panelMenuBeginY, ScreenWidth(), _panelMenuHeight,
               name.c_str(), ALIGN_CENTER);
  ...
  _contentRect = iRect(0, _panelMenuHeight, ScreenWidth(),
                       (ScreenHeight() - _panelMenuHeight), 0);
...
int MainMenu::createMenu(bool filePicker, bool loggedIn,
                         iv_menuhandler handler) {
  imenu mainMenu[] = {
      {ITEM_HEADER, 0, _menu, NULL},
      // show logged in
      {loggedIn ? (short)ITEM_ACTIVE : (short)ITEM_HIDDEN, 101, _syncFolder,
       NULL},
      {loggedIn ? (short)ITEM_ACTIVE : (short)ITEM_HIDDEN, 103, _sortBy, NULL},
      {loggedIn ? (short)ITEM_ACTIVE : (short)ITEM_HIDDEN, 104, _excludeFiles,
       NULL},
      // show if filePicker is shown
      {filePicker ? (short)ITEM_ACTIVE : (short)ITEM_HIDDEN, 105, _chooseFolder,
       NULL},
      // show always
