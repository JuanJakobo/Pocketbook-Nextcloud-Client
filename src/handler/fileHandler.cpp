...
FileHandler::FileHandler() {
  ...
  parseConfig(Util::getConfig<string>("ex_pattern", ""),
              Util::getConfig<string>("ex_folderPattern", ""),
              Util::getConfig<string>("ex_extensionList", ""),
              Util::getConfig<int>("ex_invertMatch", 0));
}

void FileHandler::parseConfig(string regex, string folderRegex,
                              string extensions, int invertMatch) {
  _extensions.clear();
  ...
  if (!regex.empty()) {
    try {
      ...
      Log::writeErrorLog("Unable to parse regex '" + regex +
                         "' for file: " + err.what());
    }
  } else {
    _useRegex = false;
  ...
  if (!folderRegex.empty()) {
    try {
      ...
      Log::writeErrorLog("Unable to parse regex '" + folderRegex +
                         "' for folder: " + err.what());
    }
  } else {
...
FileHandler::~FileHandler() {
  _instances.erase(std::remove(_instances.begin(), _instances.end(), this),
                   _instances.end());
}
...
bool FileHandler::excludeFile(std::string filename) {
  ...
  if (!_extensions.empty()) {
    ...
    if (indexOfDot != std::string::npos && filename.length() > indexOfDot + 1) {
      ...
      if (std::find(_extensions.begin(), _extensions.end(), extension) !=
          _extensions.end()) {
        return !_invertMatch;
      }
    }
...
HideState FileHandler::getHideState(Itemtype itemType, std::string prefix,
...
void FileHandler::update(string regex, string folderRegex, string extensions,
                         int invertMatch) {
