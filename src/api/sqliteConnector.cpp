...
void SqliteConnector::runMigration(int currentVersion) {
  ...
  char *log;
  sprintf(log,
          "Running migration from db version %i to %i (Program version %s)",
          currentVersion, DBVERSION, PROGRAMVERSION);
  ...
  rs = sqlite3_prepare_v2(_db, "INSERT INTO 'version' (dbversion) VALUES (?)",
                          -1, &stmt, 0);
  ...
  if (rs != SQLITE_DONE) {
    // this is critical
    Log::writeErrorLog(std::string("error inserting into version") +
                       sqlite3_errmsg(_db) + std::string(" (Error Code: ") +
                       std::to_string(rs) + ")");
  }
...
int SqliteConnector::getDbVersion() {
  ...
  int version;
  rs = sqlite3_prepare_v2(_db, "SELECT MAX(dbversion) FROM 'version'", -1,
                          &stmt, 0);
  ...
  if (version != 0) {
    ...
    // doesn't exist
    rs = sqlite3_exec(_db,
                      "ALTER TABLE metadata ADD hide INT DEFAULT 0 NOT NULL",
                      NULL, 0, NULL);
...
bool SqliteConnector::open() {
  ...
  rs = sqlite3_exec(
      ...
      NULL, 0, NULL);
  rs = sqlite3_exec(_db, "CREATE TABLE IF NOT EXISTS version (dbversion INT)",
                    NULL, 0, NULL);
...
string SqliteConnector::getEtag(const string &path) {
  ...
  rs = sqlite3_prepare_v2(
      _db, "SELECT etag FROM 'metadata' WHERE path = ? LIMIT 1;", -1, &stmt, 0);
...
FileState SqliteConnector::getState(const string &path) {
  ...
  rs = sqlite3_prepare_v2(
      _db, "SELECT state FROM 'metadata' WHERE path = ? LIMIT 1;", -1, &stmt,
      0);
  rs = sqlite3_bind_text(stmt, 1, path.c_str(), path.length(), NULL);
...
bool SqliteConnector::updateState(const string &path, FileState state) {
  ...
  rs = sqlite3_prepare_v2(_db, "UPDATE 'metadata' SET state=? WHERE path=?", -1,
                          &stmt, 0);
  ...
  if (rs != SQLITE_DONE) {
    Log::writeErrorLog(sqlite3_errmsg(_db) + std::string(" (Error Code: ") +
                       std::to_string(rs) + ")");
  }
  rs = sqlite3_clear_bindings(stmt);
  rs = sqlite3_reset(stmt);
...
std::vector<WebDAVItem>
SqliteConnector::getItemsChildren(const string &parentPath) {
  open();
  ...
  rs = sqlite3_prepare_v2(
      ...
      -1, &stmt, 0);
  rs =
      sqlite3_bind_text(stmt, 1, parentPath.c_str(), parentPath.length(), NULL);
  rs =
      sqlite3_bind_text(stmt, 2, parentPath.c_str(), parentPath.length(), NULL);

  const string storageLocation =
      NEXTCLOUD_ROOT_PATH + _fileHandler->getStorageUsername() + "/";
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    WebDAVItem temp;
    ...
    temp.lastEditDate = Util::webDAVStringToTm(
    ...
    if (temp.hide == HideState::INOTDEFINED) {
      temp.hide = _fileHandler->getHideState(temp.type, storageLocation,
...
void SqliteConnector::deleteChild(const string &path, const string &title) {
  ...
  sqlite3_stmt *stmt = 0;
  rs = sqlite3_prepare_v2(
      _db, "DELETE FROM 'metadata' WHERE path = ? AND title = ?", -1, &stmt, 0);
  rs = sqlite3_bind_text(stmt, 1, path.c_str(), path.length(), NULL);
  ...
  if (rs != SQLITE_DONE) {
    Log::writeErrorLog(
        std::string("An error ocurred trying to delete the item ") +
        sqlite3_errmsg(_db) + std::string(" (Error Code: ") +
        std::to_string(rs) + ")");
  }
  rs = sqlite3_clear_bindings(stmt);
...
void SqliteConnector::deleteItemsNotBeginsWith(string beginPath) {
  ...
  sqlite3_stmt *stmt = 0;
  rs = sqlite3_prepare_v2(
      _db, "DELETE FROM 'metadata' WHERE path NOT LIKE ? ESCAPE '#'", -1, &stmt,
      0);
  ...
  if (rs != SQLITE_DONE) {
    Log::writeErrorLog(
        std::string(
            "An error ocurred trying to delete the items that begins with " +
            beginPath) +
        sqlite3_errmsg(_db) + std::string(" (Error Code: ") +
        std::to_string(rs) + ")");
  }
...
void SqliteConnector::deleteChildren(const string &parentPath) {
  ...
  int rs;
  sqlite3_stmt *stmt = 0;
  rs = sqlite3_prepare_v2(_db, "DELETE FROM 'metadata' WHERE parentPath like ?",
                          -1, &stmt, 0);
  rs =
      sqlite3_bind_text(stmt, 1, parentPath.c_str(), parentPath.length(), NULL);
  ...
  if (rs != SQLITE_DONE) {
    Log::writeErrorLog(
        std::string("An error ocurred trying to delete items of the path ") +
        sqlite3_errmsg(_db) + std::string(" (Error Code: ") +
        std::to_string(rs) + ")");
  }
  rs = sqlite3_clear_bindings(stmt);
...
bool SqliteConnector::saveItemsChildren(const std::vector<WebDAVItem> &items) {
  ...
  // rs = sqlite3_prepare_v2(_db, "INSERT INTO 'metadata' (title, localPath,
  // path, size, parentPath, etag, fileType, lastEditDate, type, state, key)
  // VALUES (?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(key) DO UPDATE SET etag=?,
  // size=?, lastEditDate=? WHERE metadata.etag <> ?;", -1, &stmt, 0);
  ...
  for (auto item : items) {
    ...
    rs = sqlite3_bind_text(stmt, 1, item.title.c_str(), item.title.length(),
                           NULL);
    rs = sqlite3_bind_text(stmt, 2, item.localPath.c_str(),
                           item.localPath.length(), NULL);
    rs =
        sqlite3_bind_text(stmt, 3, item.path.c_str(), item.path.length(), NULL);
    rs =
        sqlite3_bind_text(stmt, 4, item.size.c_str(), item.size.length(), NULL);
    rs = sqlite3_bind_text(stmt, 5, parent.c_str(), parent.length(), NULL);
    rs =
        sqlite3_bind_text(stmt, 6, item.etag.c_str(), item.etag.length(), NULL);
    rs = sqlite3_bind_text(stmt, 7, item.fileType.c_str(),
                           item.fileType.length(), NULL);
    string lastEditDateString = Util::webDAVTmToString(item.lastEditDate);
    rs = sqlite3_bind_text(stmt, 8, lastEditDateString.c_str(),
                           lastEditDateString.length(), NULL);
    ...
    if (rs == SQLITE_CONSTRAINT) {
      ...
      rs = sqlite3_bind_text(stmt, 2, item.etag.c_str(), item.etag.length(),
                             NULL);
      string lastEditDateString = Util::webDAVTmToString(item.lastEditDate);
      rs = sqlite3_bind_text(stmt, 3, lastEditDateString.c_str(),
                             lastEditDateString.length(), NULL);
      rs = sqlite3_bind_text(stmt, 4, item.size.c_str(), item.size.length(),
                             NULL);
      rs = sqlite3_bind_text(stmt, 5, item.path.c_str(), item.path.length(),
                             NULL);
      rs = sqlite3_step(stmt);
      ...
      Log::writeErrorLog(std::string("error inserting into table ") +
                         sqlite3_errmsg(_db) + std::string(" (Error Code: ") +
                         std::to_string(rs) + ")");
    }
    rs = sqlite3_clear_bindings(stmt);
