...
std::vector<FileItem> FileBrowser::getFileStructure(const std::string &path,
  ...
  if (iv_access(localPath.c_str(), R_OK) == 0) {
    for (const auto &entry : fs::directory_iterator(localPath)) {
      ...
      auto time =
          std::chrono::system_clock::to_time_t(fs::last_write_time(entry));
      ...
      if (is_directory(entry)) {
        ...
        temp.name = temp.path.substr(temp.path.find_last_of('/') + 1,
                                     temp.path.length());
        temp.type = Type::FFOLDER;
        ...
        if (!_fileHandler->excludeFolder(
                directoryPath.substr(0, directoryPath.length())) ||
