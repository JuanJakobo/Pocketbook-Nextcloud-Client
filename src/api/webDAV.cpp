...
vector<WebDAVItem> WebDAV::getDataStructure(const string &pathUrl) {
  ...
  if (!xmlItem.empty()) {
    ...
    while (begin != std::string::npos) {
      ...
      tempItem.lastEditDate = Util::webDAVStringToTm(
          Util::getXMLAttribute(xmlItem, "d:getlastmodified"));
      ...
      if (tempItem.path.back() == '/') {
        tempItem.localPath =
            tempItem.localPath.substr(0, tempItem.localPath.length() - 1);
      ...
      tempItem.title = tempItem.title.substr(
...
bool WebDAV::get(WebDAVItem &item) {
  if (item.state == FileState::ISYNCED) {
    UpdateProgressbar(
        ("The newest version of file " + item.path + " is already downloaded.")
            .c_str(),
        0);
    return false;
  }
  ...
  if (curl) {
    ...
    if (res == CURLE_OK) {
      ...
      switch (response_code) {
      case 200:
        Log::writeInfoLog("finished download of " + item.title + " to " +
                          item.localPath);
        return true;
        break;
      case 401:
      ...
      if (res == 60)
        response =
            "Seems as if you are using Let's Encrypt Certs. Please follow the "
            "guide on Github "
